...
describe('@kind: ', ()=> {
  let doc = readDoc('TagKind.html');

  it('shows summary.', ()=>{
    assert.includes(doc, '[data-s="summaryClassDocs"] [data-s="target"]:nth-child(1)', 'Class1()');
    assert.includes(doc, '[data-s="summaryMemberDocs"] [data-s="target"]:nth-child(1)', 'member1');
  ...
  it('shows detail.', ()=>{
    assert.includes(doc, '[data-s="member"] #static-member1', 'member1');
